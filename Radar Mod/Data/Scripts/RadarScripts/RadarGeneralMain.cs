...
{
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_RadioAntenna), false, "GeneralRadarLarge", "GeneralRadarSmall")]
    class RadarGeneralMain : MyGameLogicComponent
    {
        // Countermeasure configs
        const float countermeasureEffectiveRange = 1600;

        // Note these stack on top of a 1, so 0.2 in the final will be 1.2, just needs to be this way for the math to work correctly
        const float chaffEffectiveness = 0.17f;
        const float flareEffectiveness = 0.08f;

        // Module config
        const float powerMultiplierPerModule = 1.9f;

        // Base modules 
        const float rangeMultiplierSmall = 1.2f;
        const float rangeMultiplier = 1.3f;

        const float accuracyMultiplierSmall = 0.7f;
        const float accuracyMultiplier = 0.5f;

        const float countermeasureMultiplierSmall = 0.7f;
        const float countermeasureMultiplier = 0.5f;

        const float stealthMultiplierSmall = 1.5f;
        const float stealthMultiplier = 2f;

        const float standPowerRequired = 4;
        const float standDetectionDistance = 5000;
        const float standInaccuracy = 40;
            ...
            if (radarBlock.Enabled == true || radarBlock.IsFunctional)
            {
                ...
                foreach (var entity in entities)
                {
                    if (entity is VRage.Game.ModAPI.IMyCubeGrid)
                    {
                        ...
                        if (radarToEntity.Length() < gridDetectionDistance && gridEntity != radarBlock.CubeGrid)
                        {
        ...
        public void CalculatePowerUsage(VRage.Game.ModAPI.IMyCubeGrid grid)
        {
            ...
            foreach (var block in grid.GetFatBlocks<IMyFunctionalBlock>())
            {
                ...
                if (block.IsFunctional && block.Enabled == true)
                {
                    ...
                    if (blockID == "GeneralRadarRangeUpgradeSmall" || blockID == "GeneralRadarRangeUpgrade")
                    {
                        rangeUpgrades.Add(block);

                    }
                    else if (blockID == "GeneralRadarAccuracyUpgradeSmall" || blockID == "GeneralRadarAccuracyUpgrade")
                    {
                        accuracyUpgrades.Add(block);

                    }
                    else if (blockID == "GeneralRadarFOFSmall" || blockID == "GeneralRadarFOF")
                    {
                        FOFUpgrades.Add(block);
                    }
                    else if (blockID == "GeneralRadarAdvancedTargettingSmall" || blockID == "GeneralRadarAdvancedTargetting")
                    {
                        advancedTargettingUpgrades.Add(block);
                    }
                    else if (blockID == "GeneralRadarCountermeasureUpgradeSmall" || blockID == "GeneralRadarCountermeasureUpgrade")
                    {
                        countermeasureUpgrades.Add(block);
                    }
                    else if (blockID == "GeneralRadarStealthUpgradeSmall" || blockID == "GeneralRadarStealthUpgrade")
                    {
            ...
            totalUpgradeModules = rangeUpgrades.Count + accuracyUpgrades.Count + countermeasureUpgrades.Count + stealthUpgrades.Count;    
